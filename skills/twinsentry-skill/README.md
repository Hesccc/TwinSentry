...
- `action_agent_skill.py`: **处置 Agent 技能包**。封装了获取和提交处置记录的逻辑。
...
2. **填写凭据（如果直接使用原生 Python 脚本）**：
   在 `analysis_agent_skill.py` / `action_agent_skill.py` 顶部填入您的 TwinSentry 地址和 API Key。
3. **独立运行演示模式**：
   您可以直接运行脚本体验模拟的交互流程：
   ```bash
   python analysis_agent_skill.py
   python action_agent_skill.py
   ```
